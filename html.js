...
// The following code is origially by Jerzy Głowacki, this is a modified version. 
...
(function () {
  ...
  if (urlVal) {
    ...
    githubUrl = 'https://github.com/' + user + '/' + repo + '/blob/' + branch + '/' + file;
  }
  
  var previewForm = document.getElementById('previewform');

  var url = githubUrl.replace(/\/\/github\.com/, '//raw.githubusercontent.com').replace(/\/blob\//, '/'); //Get URL of the raw file
  ...
  var replaceAssets = function () {
    ...
    for (i = 0; i < frame.length; ++i) {
      ...
      if (src.indexOf('//raw.githubusercontent.com') > 0) { //Check if it's from raw.github.com
    ...
    for (i = 0; i < a.length; ++i) {
      ...
      } else if (href.indexOf('//raw.githubusercontent.com') > 0 && (href.indexOf('.html') > 0 || href.indexOf('.htm') > 0)) { //Check if it's from raw.github.com and to HTML files
    ...
    for (i = 0; i < link.length; ++i) {
      ...
      if (href.indexOf('//raw.githubusercontent.com') > 0) { //Check if it's from raw.github.com
  ...
  var loadHTML = function (data) {
    if (data) {
      data = data.replace(/<head([^>]*)>/i, '<head$1><base href="' + url + '">')
        // .replace(/<script(\s*src=["'][^"']*["'])?(\s*type=["'](text|application)\/javascript["'])?/gi, '<script type="text/htmlpreview"$1'); //Add <base> just after <head> and replace <script type="text/javascript"> with <script type="text/htmlpreview">
      setTimeout(function () {
        document.open();
        document.write(data);
        document.close();
  ...
  var loadJS = function (data) {
    if (data) {
      ...
      if (data.includes('import {') || data.includes('from \'')) {
